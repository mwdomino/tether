...
**Tech Stack:** Go 1.23, `github.com/spf13/cobra` for CLI, `github.com/hashicorp/yamux` for connection multiplexing, `log/slog` for logging, goreleaser for release artifacts, GitHub Actions for CI.

## Global Constraints

- Single Go module rooted at `github.com/mwdomino/tether`.
- Go version: 1.23 (set in `go.mod`).
...
// suppress unused import warnings if strings goes unused after edits
...
func TestHostOpenURLNoLoopback(t *testing.T) {
	...
	// Use json to suppress unused-import lint if test grows.
...
func classifyHostError(msg string) error {
	...
	case len(msg) >= len("port ") && msg[:len("port ")] == "port ":
		return fmt.Errorf("%w: %s", ErrPortCollision, msg)
	case len(msg) >= len("browser launch") && msg[:len("browser launch")] == "browser launch":
