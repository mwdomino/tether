...
**Goal:** Ship a single Go binary `tether` with `host` and `open` subcommands that lets a headless server delegate browser-open to a host machine over an SSH `RemoteForward`, transparently tunneling OAuth/SSO localhost callbacks back to the headless side.

**Architecture:** Two roles in one binary. The host runs `tether host` as a long-lived daemon listening on a unix socket (or loopback TCP on Windows) exposed inside the agent via SSH `RemoteForward`. The headless side invokes `tether open <url>` per call; each invocation opens a yamux-multiplexed connection carrying a JSON control request and zero-or-more raw-byte tunnel substreams that proxy host-side localhost connections back to the headless side.
...
- Exit codes — 0: success, 2: host port bind collision, 3: host unreachable, 4: auth mismatch, 5: timeout, 6: browser launch failed.
...
| `internal/host/tunnel.go` | One host-side TCP listener per loopback port; opens substreams toward agent; grace-period release. |
| `internal/agent/agent.go` | Agent: dial host, yamux client, send request, optionally accept tunnel substreams and relay to local. |
| `internal/install/install.go` | Service install/uninstall orchestration; `runtime.GOOS` switch to per-OS implementations. |
| `cmd/tether/cmd_install.go` | `tether install` cobra command. |
| `cmd/tether/cmd_uninstall.go` | `tether uninstall` cobra command. |
...
func main() {
	root := &cobra.Command{
		Use:           "tether",
		Short:         "Open URLs requested on a headless server in a browser on your host.",
		Version:       Version,
		SilenceUsage:  true,
		SilenceErrors: true,
	}
...
func newHostCmd() *cobra.Command {
	c := &cobra.Command{
		Use:   "host",
		Short: "Run the host-side daemon that opens browsers and relays loopback callbacks.",
...
func newOpenCmd() *cobra.Command {
	c := &cobra.Command{
		Use:   "open <url>",
		Short: "Send a URL to the host host to be opened in a browser.",
...
- Produces:
  ...
  - Extension of `(*Host).handleConn` so that when `req.LoopbackPorts` is non-empty:
    1. Bind `127.0.0.1:<port>` for each port on the host side.
    2. On any bind failure, release all already-bound listeners and reply with `{"ok":false,"error":"port <p> already in use on host"}`.
...
// tunnelMgr binds host loopback ports and opens substreams back to the
...
git commit -m "feat: SSO loopback tunneling — host binds host ports and relays callbacks"
```

---

### Task 6: `tether install` / `tether uninstall` — service management

**Files:**
- Create: `internal/install/install.go`
- Create: `internal/install/install_test.go`
- Create: `cmd/tether/cmd_install.go`
- Create: `cmd/tether/cmd_uninstall.go`
- Modify: `cmd/tether/main.go` (register the two new subcommands)

**Interfaces:**
- Consumes: nothing from `internal/host` or `internal/agent` — install is independent.
- Produces:
  - `install.Install(binaryPath string) error` — writes the per-OS unit, enables it, and starts it. Idempotent: an existing unit is overwritten; a running unit is restarted.
  - `install.Uninstall() error` — stops the unit (if running) and removes the file. Idempotent: missing file = success.
  - `install.UnitPath() (string, error)` — returns the absolute path where the unit lives, for diagnostics and tests.
- Per-OS behavior (runtime.GOOS switch inside `install.go`):
  - **linux:** systemd user unit at `~/.config/systemd/user/tether-host.service`. After write: `systemctl --user daemon-reload && systemctl --user enable --now tether-host.service`. Uninstall: `systemctl --user disable --now tether-host.service` (ignore exit code if not found), then remove file, then `daemon-reload`.
  - **darwin:** launchd LaunchAgent plist at `~/Library/LaunchAgents/com.tether.host.plist`. Install: write plist, then `launchctl bootstrap gui/$UID <path>`; fall back to `launchctl load -w <path>` if `bootstrap` errors (older macOS). Uninstall: `launchctl bootout gui/$UID/com.tether.host` then remove file (best-effort).
  - **windows:** Startup folder script at `%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\tether-host.cmd` containing `start "" "<abs-path-to-tether.exe>" host`. Install also launches the host immediately via the same `start ""` invocation (detached, no console window via the `start` builtin). Uninstall: best-effort `taskkill /F /IM tether.exe` and remove the .cmd file.

**Implementation notes:**
- The binary path used by the unit is the absolute path of the current executable via `os.Executable()` (resolved through any symlinks via `filepath.EvalSymlinks`).
- The Linux systemd unit body:
  ```
  [Unit]
  ...
  ```
- The macOS launchd plist body (the standard `Label`, `ProgramArguments`, `RunAtLoad`, `KeepAlive` keys; XML).
- The Windows .cmd body: single-line `start "" "<BIN_PATH>" host`.
- Cobra commands have no flags beyond `--help`.
- Uninstall must be safe to run when nothing is installed (no error, exit 0).
...
Expected: build failure — `UnitPath`, `renderUnit`, `Uninstall`, `unitPathOverride` undefined.

- [ ] **Step 3: Implement `internal/install/install.go`**
...
- [ ] **Step 4: Verify the install tests pass**

Run: `go test ./internal/install/... -v`
Expected: `TestUnitPathPerOS`, `TestRenderUnitContainsBinary`, `TestUninstallMissingIsNoop` PASS.

- [ ] **Step 5: Wire `tether install` subcommand**
...
- [ ] **Step 6: Wire `tether uninstall` subcommand**
...
- [ ] **Step 7: Register the new subcommands in `cmd/tether/main.go`**

Modify the `root.AddCommand(...)` call so both new commands are registered:

```go
root.AddCommand(newHostCmd(), newOpenCmd(), newInstallCmd(), newUninstallCmd())
```

- [ ] **Step 8: Verify the full build and suite**
...
./tether --help
./tether install --help
./tether uninstall --help
go test ./...
```
Expected: build clean; `--help` lists `host`, `open`, `install`, `uninstall`; full suite passes.

- [ ] **Step 9: Commit**

```bash
git add internal/install cmd/tether/cmd_install.go cmd/tether/cmd_uninstall.go cmd/tether/main.go
git commit -m "feat(install): tether install/uninstall for per-user host service"
```

---

### Task 7: Release pipeline — goreleaser config and tag workflow
